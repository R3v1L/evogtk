...
class ThreadTask(object):
    ...
    """
    def __init__(self,type,task_callback,gui_callback=None,end_callback=None,gui_delay=None):
        """
    ...
    def __run_simple(self,*args,**kwargs):
        ...
        if self.__gui_callback:
            if self.__gui_delay:
                gobject.timeout_add(self.__gui_delay,self.__gui_callback,priority=gobject.PRIORITY_HIGH)
            else:
                gobject.idle_add(self.__gui_callback)
        resp=self.__task_callback(*args,**kwargs)
        if self.__end_callback:
            self.__end_callback(resp)
    ...
    def __run_daemon(self,*args,**kwargs):
        ...
        while self.__daemon_running:
            resp=self.__task_callback(*args,**kwargs)
        if self.__end_callback:
            return self.__end_callback(resp)
