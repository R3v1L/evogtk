...
class _PreferencesHelper(object):
    ...
    def __init__(self,optionstruct,configfile,guidata):
        ...
        if os.name == 'posix':
            ...
            pathformat='%s/%s'
        self.__configfile=os.path.normpath(pathformat % (os.path.expanduser('~'),configfile))
    ...
    def load(self,filename=None):
        ...
        if not filename:
            if self.__configfile:
                ...
                raise Exception(_('EVOGTK: Need a filename for loading preferences'))
    ...
    def add_section(self,name,values):
        """
        Add a new section to preferences
        """
        if not self.__config.has_section(name):
            ...
            raise Exception(_('EVOGTK: Section "%s" already exists in this preferences instance') % name)
    
    def get_section(self,name):
        """
        Returns a section in a dictionary format
        """
    ...
    def remove_section(self,name):
        """
        Removes a section from preferences
        """
        if self.__config.has_section(name):
...
class _PreferencesSection(object):
    ...
    """
    __supported_types=['str','int','float','bool','list']
    ...
    def get_option(self,name):
        ...
        try:
            if type=='str':
                ...
                value=self.__config.get(self.__section,name).strip().split(opt[3])
        except:
            return defaultvalue
        return value
    ...
    def set_option(self,name,value):
        ...
        if type=='str':
            ...
            self.__config.set(self.__section,name,opt[3].join(value))
        # Set linked widgets
