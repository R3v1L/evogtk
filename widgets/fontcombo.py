...
#        arrowup=gtk.Arrow(gtk.ARROW_UP,gtk.SHADOW_ETCHED_IN)
#        arrowdown=gtk.Arrow(gtk.ARROW_DOWN,gtk.SHADOW_ETCHED_IN)
...
#        fixedarrows.put(arrowdown,0,8)
    ...
    def __popdown(self,widget=None,event=None,data=None):
        """
        Hides popup window
        """
        alloc=widget.get_allocation()
        if not event or (event and event.x < 0 or event.y < 0 or event.x > alloc.width or event.y > alloc.height):
            self.__popup_window.hide()
            if self.__hide_signal:
